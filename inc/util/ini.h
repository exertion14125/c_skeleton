...
// ini_mem_store.header
#ifndef INI_MEM_STORE_H 
#define INI_MEM_STORE_H

#define INI_MAX_CONFIGS   128 // Maximum number of key-value pairs in memory store
#define INI_MAX_SECTION   32  // Maximum length of section names
#define INI_MAX_NAME      32  // Maximum length of key names
#define INI_MAX_VALUE     128 // Maximum length of value strings
...
typedef struct {
   char section[INI_MAX_SECTION];  // Section name
   char key[INI_MAX_NAME];         // Key name
   char value[INI_MAX_VALUE];      // Value string
} ini_kv_pair_t;

/* INI memory store */
typedef struct {
   int count;                              // Number of stored entries
   ini_kv_pair_t items[INI_MAX_CONFIGS]; // Array of key-value entries
} ini_store_t;

/* Load all INI key-values into memory store */
int ini_load_to_memory(const char* filename, ini_store_t* store);

/* Lookup raw string value from memory store */
const char* ini_mem_lookup(const ini_store_t* store, const char* section, const char* key);

/* Typed accessors from memory store */
int ini_mem_get_int_value(const ini_store_t* store, const char* section, const char* key, int default_val);
void ini_mem_get_string_value(const ini_store_t* store, const char* section, const char* key, char* out, size_t max_len, const char* default_val);
...
/* Setters to memory store (update if exists, add if not) */
