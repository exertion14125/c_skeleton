...
#define SYS_THREAD_STACK_SIZE         (64 * 1024) //64Kbyte
...
int sys_mgr_start_runloop(sys_mgr_t *m)
{
        ...
        pthread_attr_setstacksize(&pth_attr, SYS_THREAD_STACK_SIZE);
        if (pthread_create(&m->runloop_tid, &pth_attr, sys_mgr_runloop_thread_main, m) != 0) {
                m->runloop_run = 0U;
                return -errno;
        }
